...
class SchedulesIntTest : SqsIntegrationTestBase() {
  ...
  inner class UpdateScheduleInstances {
    ...
    fun `should update scheduled instances`() {
      ...
      await untilAsserted { ActivitiesApiExtension.activitiesApi.verify(WireMock.getRequestedFor(urlEqualTo("/schedules/$ACTIVITY_SCHEDULE_ID"))) }
      await untilAsserted { MappingExtension.mappingServer.verify(WireMock.getRequestedFor(urlEqualTo("/mapping/activities/activity-schedule-id/$ACTIVITY_SCHEDULE_ID"))) }
      await untilAsserted {
        NomisApiExtension.nomisApi.verify(
          WireMock.putRequestedFor(urlEqualTo("/activities/$NOMIS_CRS_ACTY_ID/schedules"))
      ...
      Assertions.assertThat(awsSqsActivityDlqClient.countAllMessagesOnQueue(activityDlqUrl).get()).isEqualTo(0)
    }
