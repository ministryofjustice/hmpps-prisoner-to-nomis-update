...
@Import(CaseNotesMappingApiService::class, CaseNotesMappingApiMockServer::class, RetryApiService::class)
class CaseNotesMappingApiServiceTest {
  ...
  @Nested
  inner class GetByPrisoner {
    private val offenderNo = "A1234AA"

    @Test
    fun `will pass oath2 token to service`() = runTest {
      casenotesMappingApiMockServer.stubGetByPrisoner(offenderNo)

      apiService.getByPrisoner(offenderNo)
    ...
    fun `will pass NOMIS ids to service`() = runTest {
      ...
      casenotesMappingApiMockServer.verify(
        getRequestedFor(urlPathEqualTo("/mapping/casenotes/$offenderNo/all")),
      )
    }

    @Test
    fun `will return case note when mapping exists`() = runTest {
      casenotesMappingApiMockServer.stubGetByPrisoner(
        offenderNo,
        AllPrisonerCaseNoteMappingsDto(
          listOf(
            CaseNoteMappingDto(
              dpsCaseNoteId = offenderNo,
              nomisBookingId = 123456,
              offenderNo = "A1234AA",
              nomisCaseNoteId = 1,
              mappingType = CaseNoteMappingDto.MappingType.MIGRATED,
      ...
      val mapping = apiService.getByPrisoner(offenderNo).mappings.firstOrNull()

      assertThat(mapping?.nomisBookingId).isEqualTo(123456)
      assertThat(mapping?.nomisCaseNoteId).isEqualTo(1)
      assertThat(mapping?.dpsCaseNoteId).isEqualTo(offenderNo)
      assertThat(mapping?.offenderNo).isEqualTo("A1234AA")
    }

    @Test
    fun `will throw error when API returns an error`() = runTest {
      casenotesMappingApiMockServer.stubGetByPrisoner(INTERNAL_SERVER_ERROR)
