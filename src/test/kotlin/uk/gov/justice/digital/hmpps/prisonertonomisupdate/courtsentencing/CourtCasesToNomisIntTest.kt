  ...
  inner class CreateCourtAppearance {
    ...
    inner class WhenCourtAppearanceHasBeenCreatedInDPS {
      ...
      inner class WhenAppearanceOnOldBooking {
        val clonedCourtCaseResponse = nomisCourtAppearanceCreateResponse().copy(
        ...
        @Nested
        inner class FutureHearing {
          @BeforeEach
          fun setUp() {
            courtSentencingNomisApi.stubCourtAppearanceCreateSuccessAfterError(
              OFFENDER_NO,
              NOMIS_COURT_CASE_ID_FOR_CREATION,
              errorStatus = FAILED_DEPENDENCY,
              errorResponse = ErrorResponse(
                status = FAILED_DEPENDENCY.value(),
                userMessage = "Cannot add future dated appearance until case has been cloned to latest booking",
                developerMessage = "Cannot add future dated appearance until case has been cloned to latest booking",
                moreInfo = "1234",
              ),
              clonedCourtCaseResponse,
            )
            publishCreateCourtAppearanceDomainEvent(isOnFutureCourtAppearance = true).also {
              waitForAnyProcessingToComplete("court-appearance-create-cases-cloned")
            }
          }

          @Test
          fun `will call nomis api to create the Court Appearance specifying a future appearance`() {
            val request: CourtAppearanceRequest = NomisApiMockServer.getRequestBody(postRequestedFor(urlEqualTo("/prisoners/$OFFENDER_NO/sentencing/court-cases/${NOMIS_COURT_CASE_ID_FOR_CREATION}/court-appearances")))
            assertThat(request.futureAppearance).isTrue()
          }

          @Test
          fun `will initially fail with incorrect dependent state`() {
            verify(telemetryClient).trackEvent(
              eq("court-appearance-create-awaiting-dependent-state"),
              any(),
              isNull(),
            )
          }
          ...
          @Test
          fun `will eventual create success telemetry`() {
            verify(telemetryClient).trackEvent(
              eq("court-appearance-create-success"),
              any(),
              isNull(),
          ...
          @Test
          fun `will eventual create cloned telemetry`() {
            verify(telemetryClient).trackEvent(
              eq("court-appearance-create-cases-cloned"),
              check {
                assertThat(it["offenderNo"]).isEqualTo(OFFENDER_NO)
          ...
          @Test
          fun `will eventual create mapping`() {
            val request: CourtCaseBatchUpdateAndCreateMappingDto =
              CourtSentencingMappingApiMockServer.getRequestBody(putRequestedFor(urlEqualTo("/mapping/court-sentencing/court-cases/update-create")))
            assertThat(request.mappingsToCreate.courtAppearances).hasSize(1)
          }
  ...
  inner class UpdateCharge {
    ...
    inner class WhenCourtChargeMappingDoesNotExist {
      ...
      fun setUp() {
        ...
        courtSentencingMappingApi.stubGetCourtCaseMappingGivenDpsId(COURT_CASE_ID_FOR_CREATION, nomisCourtCaseId = NOMIS_COURT_CASE_ID_FOR_CREATION)
      ...
      @Test
      fun `will not update an court charge in NOMIS`() {
        waitForAnyProcessingToComplete(4)
        verify(telemetryClient, times(3)).trackEvent(
          eq("charge-updated-awaiting-parent"),
          check {
            assertThat(it["dpsCourtCaseId"]).isEqualTo(COURT_CASE_ID_FOR_CREATION)
            assertThat(it["dpsCourtAppearanceId"]).isEqualTo(DPS_COURT_APPEARANCE_ID)
            assertThat(it["offenderNo"]).isEqualTo(OFFENDER_NO)
            assertThat(it["dpsChargeId"]).isEqualTo(DPS_COURT_CHARGE_ID)
            assertThat(it["reason"]).isEqualTo("Missing Dps charge id: $DPS_COURT_CHARGE_ID")
          },
          isNull(),
        )
    ...
    @Nested
    inner class WhenCourtAppearanceMappingDoesNotExist {
    ...
    @Nested
    inner class WhenCourtChargeEntityDoesNotExist {

      @BeforeEach
      fun setUp() {
        courtSentencingApi.stubGetCourtChargeByAppearanceGetError(
          courtAppearanceId = DPS_COURT_APPEARANCE_ID,
          courtChargeId = DPS_COURT_CHARGE_ID,
        )
        publishUpdatedCourtChargeDomainEvent()
      ...
      fun `will not update an court charge in NOMIS`() {
        ...
        verify(telemetryClient).trackEvent(
          ...
          check {
            ...
            assertThat(it["offenderNo"]).isEqualTo(OFFENDER_NO)
            assertThat(it["dpsChargeId"]).isEqualTo(DPS_COURT_CHARGE_ID)
            assertThat(it["reason"]).isEqualTo("Charge with id $DPS_COURT_CHARGE_ID and court appearance id $DPS_COURT_APPEARANCE_ID does not exist in RaS")
          },
          isNull(),
        )
  ...
  inner class UpdateSentence {
    ...
    inner class WhenMappingDoesntExistForSentence {
      ...
      fun `will not update a sentence in NOMIS`() {
        await untilAsserted {
          verify(telemetryClient, times(3)).trackEvent(
            eq("sentence-updated-awaiting-parent"),
            check {
              assertThat(it["dpsSentenceId"]).isEqualTo(DPS_SENTENCE_ID)
              assertThat(it["dpsCourtAppearanceId"]).isEqualTo(DPS_COURT_APPEARANCE_ID)
              assertThat(it["offenderNo"]).isEqualTo(OFFENDER_NO)
    ...
    @Nested
    inner class WhenEntityDoesntExistForSentence {
      ...
      fun setUp() {
        ...
        courtSentencingApi.stubGetSentenceGetError(sentenceId = DPS_SENTENCE_ID)
        publishUpdateSentenceDomainEvent()
      }
      ...
      fun `will not update a sentence in NOMIS`() {
        await untilAsserted {
          verify(telemetryClient, times(1)).trackEvent(
            eq("sentence-updated-ignored"),
            check {
              assertThat(it["reason"]).isEqualTo("DPS sentence $DPS_SENTENCE_ID does not exist in RaS")
              assertThat(it["dpsSentenceId"]).isEqualTo(DPS_SENTENCE_ID)
              assertThat(it["dpsCourtAppearanceId"]).isEqualTo(DPS_COURT_APPEARANCE_ID)
              assertThat(it["offenderNo"]).isEqualTo(OFFENDER_NO)
              assertThat(it["dpsCourtCaseId"]).isEqualTo(COURT_CASE_ID_FOR_CREATION)
          ...
          verify(telemetryClient, times(3)).trackEvent(
            ...
            check {
              ...
              assertThat(it["reason"])
                .isEqualTo("Missing Dps charge id: $DPS_COURT_CHARGE_ID")
            },
            isNull(),
          )
        }
    ...
    @Nested
    inner class WhenMappingDoesntExistForConsecutiveSentence {
      ...
      fun setUp() {
        ...
        // consecutive sentence mapping doesn't exist
        courtSentencingMappingApi.stubGetSentenceMappingGivenDpsIdWithError(
          DPS_SENTENCE_ID_2,
          404,
        )
        ...
        await untilAsserted {
          verify(telemetryClient, times(3)).trackEvent(
            ...
            check {
              ...
              assertThat(it["offenderNo"]).isEqualTo(OFFENDER_NO)
              assertThat(it["dpsCourtCaseId"]).isEqualTo(COURT_CASE_ID_FOR_CREATION)
              assertThat(it["reason"]).isEqualTo("Missing DPS consecutive sentence id: $DPS_SENTENCE_ID_2")
            },
            isNull(),
          )
        }
  ...
  inner class UpdatePeriodLength {
    ...
    inner class WhenMappingDoesntExistForPeriodLength {
      ...
      fun `will not update a sentence term in NOMIS`() {
        await untilAsserted {
          verify(telemetryClient, times(3)).trackEvent(
            eq("sentence-term-updated-awaiting-parent"),
            check {
              assertThat(it["dpsSentenceId"]).isEqualTo(DPS_SENTENCE_ID)
              assertThat(it["dpsTermId"]).isEqualTo(DPS_TERM_ID)
              assertThat(it["dpsCourtAppearanceId"]).isEqualTo(DPS_COURT_APPEARANCE_ID)
    ...
    @Nested
    inner class WhenEntityDoesntExistForPeriodLength {

      @BeforeEach
      fun setUp() {
        courtSentencingApi.stubGetPeriodLengthGetError(DPS_TERM_ID)
        publishUpdatePeriodLengthDomainEvent()
      }
        ...
        await untilAsserted {
          verify(telemetryClient, times(1)).trackEvent(
            ...
            check {
              ...
              assertThat(it["offenderNo"]).isEqualTo(OFFENDER_NO)
              assertThat(it["dpsCourtCaseId"]).isEqualTo(COURT_CASE_ID_FOR_CREATION)
              assertThat(it["reason"]).isEqualTo("DPS sentence term $DPS_TERM_ID does not exist in RaS")
            },
            isNull(),
          )
        }
  ...
  inner class RefreshCaseReferences {
    ...
    inner class WhenMappingDoesntExistForCourtCase {
      ...
      fun `will not update the references in NOMIS`() {
        await untilAsserted {
          verify(telemetryClient, times(3)).trackEvent(
            eq("case-references-refreshed-awaiting-parent"),
            check {
              assertThat(it["dpsCourtCaseId"]).isEqualTo(COURT_CASE_ID_FOR_CREATION)
              assertThat(it["offenderNo"]).isEqualTo(OFFENDER_NO)
              assertThat(it["reason"]).isEqualTo("Missing DPS Case: $COURT_CASE_ID_FOR_CREATION")
            },
            isNull(),
          )
        }
