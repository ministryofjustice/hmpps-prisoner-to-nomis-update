...
class CourtSchedulerAppearanceService(
  ...
  suspend fun courtAppearanceChanged(event: CourtSchedulerEvent) {
    ...
    val nomis = nomisApi.upsertCourtScheduleOut(prisonerNumber, dps.toNomisUpsertRequest())
      .also {
    ...
    if (existingMapping == null) {
      mappingApi.createCourtScheduleMapping(CourtScheduleMappingDto(prisonerNumber, nomis.bookingId, nomis.eventId, dpsCourtAppearanceId, CourtScheduleMappingDto.MappingType.DPS_CREATED))
    }

    telemetryClient.trackEvent("$TELEMETRY_KEY_CREATE-success", telemetryMap, null)
  }
}

private fun CourtEvent.toNomisUpsertRequest(): UpsertCourtScheduleOut {
