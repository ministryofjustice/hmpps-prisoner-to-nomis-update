...
import uk.gov.justice.digital.hmpps.prisonertonomisupdate.movements.transfer.TransferScheduleReconciliationService.Companion.TELEMETRY_TRANSFER_SCHEDULER
import uk.gov.justice.digital.hmpps.prisonertonomisupdate.nomisprisoner.model.PrisonerIds
import uk.gov.justice.digital.hmpps.prisonertonomisupdate.services.NomisApiService
import kotlin.collections.isNotEmpty

@Service
class TransferSchedulerReconciliationServiceActivePrisoners(
  ...
  suspend fun generateTransferSchedulerReconciliationReportBatch() {
    telemetryClient.trackEvent(
      ...
      mapOf("TYPE" to "ACTIVE"),
    )

    runCatching { generateTransferSchedulerReconciliationReport() }
      .onSuccess {
        log.info("Transfer scheduler active prisoners reconciliation report completed with ${it.mismatches.size} mismatches")
        telemetryClient.trackEvent(
      ...
      .onFailure {
        ...
        log.error("Transfer scheduler active prisoners reconciliation report failed", it)
      }
  }
  ...
  suspend fun generateTransferSchedulerReconciliationReport(): ReconciliationResult<PrisonerIds> = generateReconciliationReport(
  ...
  private suspend fun getNextBookingsForPage(lastBookingId: Long): ReconciliationPageResult<PrisonerIds> = runCatching {
    nomisPrisonerApi.getAllLatestBookings(
