...
fun List<CSIPChildMappingDto>.find(dpsId: String): CSIPChildMappingDto? = find { it.dpsId == dpsId }

fun Review.toNomisReviewRequest(nomisReviewId: Long? = null, attendeeMappings: List<CSIPChildMappingDto>? = null) = ReviewRequest(
  id = nomisReviewId,
...
fun Attendee.toNomisAttendeeRequest(nomisAttendeeId: Long? = null) = AttendeeRequest(
  id = nomisAttendeeId,
...
fun DecisionAndActions.toNomisDecisionRequest() = DecisionRequest(
...
fun DecisionAndActions.toNomisActionRequest() = ActionsRequest(
...
fun Investigation.toNomisInvestigationRequest(interviewMappings: List<CSIPChildMappingDto>? = null) = InvestigationDetailRequest(
...
fun Interview.toNomisInterviewRequest(nomisInterviewId: Long? = null) = InterviewDetailRequest(
  id = nomisInterviewId,
...
fun Referral.toNomisReportDetails(factorMappings: List<CSIPChildMappingDto>? = null) = UpsertReportDetailsRequest(
...
fun ContributoryFactor.toNomisFactorRequest(nomisFactorId: Long? = null) = CSIPFactorRequest(
  id = nomisFactorId,
...
fun IdentifiedNeed.toNomisPlanRequest(nomisPlanId: Long? = null) = PlanRequest(
  id = nomisPlanId,
