...
class CourtSentencingService(
  ...
  private val recallMappingAndDPSSynchronisationHandler: RecallMappingAndDPSSynchronisationHandler,
  ...
  suspend fun createCourtAppearance(createEvent: CourtAppearanceCreatedEvent, forceClone: Boolean = false, forcePreventClone: Boolean = false) {
    ...
    if (isDpsCreated(source)) {
      synchronise {
        ...
        transform {
          val courtCaseMapping = tryFetchParent(message = "Missing DPS Case: $dpsCourtCaseId") {
          ...
          val courtEventChargesToUpdateWithOutcomes: MutableList<CourtEventChargeRequest> = mutableListOf()
          courtAppearance.charges.forEach { charge ->
            val courtChargeMapping = tryFetchParent(message = "Missing DPS charge: ${charge.lifetimeUuid}") {
              courtCaseMappingService.getMappingGivenCourtChargeIdOrNull(charge.lifetimeUuid.toString())
            }
            courtEventChargesToUpdate.add(courtChargeMapping.nomisCourtChargeId)
            courtEventChargesToUpdateWithOutcomes.add(charge.toNomisCourtEventCharge(courtChargeMapping.nomisCourtChargeId))
          }
  ...
  private suspend fun OffenderChargeIdResponse.toCourtCaseBatchMappingDto(
    dpsChargeId: String,
    offenderNo: String,
  ): CourtCaseBatchUpdateAndCreateMappingsWrapper = CourtCaseBatchUpdateAndCreateMappingsWrapper(
    mappings =
      ...
      mappingsToCreate = CourtCaseBatchMappingDto(
        ...
        courtCharges = listOf(
          CourtChargeMappingDto(
            ...
            nomisCourtChargeId = this.offenderChargeId,
          ),
        ),
        sentences = emptyList(),
        sentenceTerms = emptyList(),
  ...
  suspend fun deleteCourtAppearance(createdEvent: CourtAppearanceCreatedEvent) {
    ...
    val offenderNo = eventOffenderNo(createdEvent.personReference)
    val telemetryMap = createdEvent.asTelemetry()
  ...
  suspend fun createCharge(createEvent: CourtChargeCreatedEvent) {
    ...
    if (isDpsCreated(source)) {
      synchronise {
        ...
        transform {
          val courtCaseMapping = tryFetchParent(message = "Missing DPS Case: $courtCaseId") {
            courtCaseMappingService.getMappingGivenCourtCaseIdOrNull(dpsCourtCaseId = courtCaseId)
          }

          log.info("Retrieving charge details for charge id $chargeId with court appearance id $courtAppearanceId")
          ...
          log.info("Charge id $chargeId with court appearance id $courtAppearanceId has an outcome code of ${charge.nomisOutcomeCode} and offence code of ${charge.offenceCode}")
          telemetryMap["nomisOutcomeCode"] = charge.nomisOutcomeCode ?: "null"
          ...
          nomisApiService.createCourtCharge(
            ...
            telemetryMap["nomisCourtCaseId"] = courtCaseMapping.nomisCourtCaseId.toString()
            response.toCourtCaseBatchMappingDto(dpsChargeId = chargeId, offenderNo = offenderNo)
          }
        }
        saveMapping {
          createChargeMappingsAndNotifyClonedCases(
            mappingsWrapper = it,
            offenderNo = offenderNo,
            telemetry = telemetryMap,
          )
  ...
  suspend fun updateCourtAppearance(createEvent: CourtAppearanceCreatedEvent) {
    ...
    val courtAppearanceId = createEvent.additionalInformation.courtAppearanceId
    val offenderNo: String = eventOffenderNo(createEvent.personReference)
    val telemetryMap: MutableMap<String, String> = createEvent.asTelemetry()
    ...
    if (isDpsCreated(source)) {
      track("court-appearance-updated", telemetryMap) {
        courtSentencingApiService.getCourtAppearanceOrNull(courtAppearanceId)?.also { dpsCourtAppearance ->
          ...
          telemetryMap["nomisCourtAppearanceId"] = courtAppearanceMapping.nomisCourtAppearanceId.toString()
          telemetryMap["nomisOutcomeCode"] = dpsCourtAppearance.nomisOutcomeCode ?: "null"
          telemetryMap["dpsAppearanceDate"] = dpsCourtAppearance.appearanceDate.toString()
          ...
          dpsCourtAppearance.charges.forEach { charge ->
            courtCaseMappingService.getMappingGivenCourtChargeIdOrNull(charge.lifetimeUuid.toString())?.also { mapping ->
              ...
              throw AwaitParentEntityRetry(
                "Missing Dps charge id: ${charge.lifetimeUuid}",
              )
            }
          }
          telemetryMap["courtEventChargesWithOutcomes"] = courtEventChargesWithOutcomesToUpdate.toString()
          ...
          telemetryMap["reason"] = "DPS court appearance $courtAppearanceId does not exist in RaS"
  ...
  suspend fun createChargeMappingsAndNotifyClonedCases(
    mappingsWrapper: CourtCaseBatchUpdateAndCreateMappingsWrapper,
    offenderNo: String,
    telemetry: Map<String, String>,
  ) {
  ...
  suspend fun createChargeMappingsRetry(message: CreateMappingRetryMessage<CourtCaseBatchUpdateAndCreateMappingsWrapper>) = with(message) {
    createChargeMappingsAndNotifyClonedCases(
  ...
  override suspend fun retryCreateMapping(message: String) {
    ...
    when (baseMapping.entityName) {
      ...
      EntityType.COURT_APPEARANCE_RECALL.displayName -> recallMappingAndDPSSynchronisationHandler.createMappingsAndNotifyDPSRetry(
        message.fromJson(),
      )

      EntityType.COURT_APPEARANCE_RECALL_UPDATE.displayName -> recallMappingAndDPSSynchronisationHandler.updateMappingsAndNotifyDPSRetry(
        message.fromJson(),
      )

      EntityType.COURT_CHARGE.displayName -> createChargeMappingsRetry(message.fromJson())
    ...
    if (isDpsCreated(source)) {
      synchronise {
        ...
        transform {
          ...
          val chargeMapping = tryFetchParent(message = "Missing DPS Charge: ${dpsSentence.chargeLifetimeUuid}") {
            courtCaseMappingService.getMappingGivenCourtChargeIdOrNull(dpsSentence.chargeLifetimeUuid.toString())
          }

          val consecutiveSentenceSeq = dpsSentence.consecutiveToLifetimeUuid?.let {
            tryFetchParent(message = "Missing DPS consecutive sentence id: $dpsSentence") {
              getNomisConsecutiveSentenceSeqOrNull(
                dpsSentence,
                telemetryMap,
              )
            }
    ...
    if (isDpsCreated(source)) {
      synchronise {
        ...
        transform {
          ...
          val sentenceMapping = tryFetchParent(message = "Missing DPS Sentence: $dpsSentenceId") {
  ...
  suspend fun createRecallSentences(recallInsertedEvent: RecallEvent) {
    ...
    val offenderNo: String = eventOffenderNo(recallInsertedEvent.personReference)
    val telemetryMap = recallInsertedEvent.asTelemetry()
    ...
    if (isDpsCreated(source) && sentenceIds.isNotEmpty()) {
      runCatching {
        ...
        recallMappingAndDPSSynchronisationHandler.tryCreateMappingsAndNotifyDPS(
  ...
  suspend fun updateRecallSentences(recallUpdateEvent: UpdateRecallEvent) {
    ...
    if (isDpsCreated(source)) {
      runCatching {
        ...
        recallMappingAndDPSSynchronisationHandler.tryUpdateMappingsAndNotifyDPS(
  ...
  suspend fun deleteRecallSentences(recallDeletedEvent: RecallEvent) {
    ...
    if (isDpsCreated(source)) {
      runCatching {
        ...
        telemetryClient.trackEvent("recall-deleted-failed", telemetryMap)
        throw e
      }
    } else {
    ...
    if (isDpsCreated(source)) {
      track("sentence-term-updated", telemetryMap) {
        courtSentencingApiService.getPeriodLengthOrNull(termId)?.also { dpsPeriodLength ->
          ...
          telemetryMap["termCode"] = dpsPeriodLength.sentenceTermCode
  ...
  suspend fun refreshCaseReferences(createEvent: CaseReferencesUpdatedEvent) {
    ...
    courtSentencingApiService.getCourtCaseOrNull(dpsCourtCaseId)?.also { dpsCourtCase ->
      courtCaseMappingService.getMappingGivenCourtCaseIdOrNull(dpsCourtCaseId = dpsCourtCaseId)
        ...
        telemetryMap["reason"] = "Missing DPS Case: $dpsCourtCaseId"
        telemetryClient.trackEvent(
          "case-references-refreshed-awaiting-parent",
          telemetryMap,
          null,
        )
        throw ParentEntityNotFoundRetry(
  ...
  private suspend fun retrieveParentCaseMapping(dpsCaseId: String): CourtCaseMappingDto {
    val mapping = courtCaseMappingService.getMappingGivenCourtCaseIdOrNull(dpsCaseId)
    return if (mapping != null) {
      mapping
    } else {
      throw AwaitParentEntityRetry(
        "Missing Dps case id: $dpsCaseId",
      )
    }
  }

  private suspend fun retrieveParentAppearanceMapping(dpsAppearanceId: String): CourtAppearanceMappingDto {
    val mapping = courtCaseMappingService.getMappingGivenCourtAppearanceIdOrNull(dpsAppearanceId)
    return if (mapping != null) {
      mapping
    } else {
      throw AwaitParentEntityRetry(
        "Missing Dps appearance id: $dpsAppearanceId",
      )
    }
  }

  private suspend fun retrieveParentChargeMapping(dpsChargeId: String): CourtChargeMappingDto {
    val mapping = courtCaseMappingService.getMappingGivenCourtChargeIdOrNull(dpsChargeId)
    return if (mapping != null) {
      mapping
    } else {
      throw AwaitParentEntityRetry(
        "Missing Dps charge id: $dpsChargeId",
      )
    }
  }
...
fun LegacyCharge.toNomisCourtCharge(isOnFutureCourtAppearance: Boolean, isBreach: Boolean): OffenderChargeRequest = OffenderChargeRequest(
...
fun LegacyCharge.toNomisCourtEventCharge(nomisChargeId: Long): CourtEventChargeRequest = CourtEventChargeRequest(
...
fun CourtSentencingService.CourtCaseCreatedEvent.asTelemetry() = mutableMapOf(
  "dpsCourtCaseId" to additionalInformation.courtCaseId,
  "offenderNo" to eventOffenderNo(personReference),
)

fun CourtSentencingService.CourtAppearanceCreatedEvent.asTelemetry() = mutableMapOf(
  "dpsCourtCaseId" to additionalInformation.courtCaseId,
  "dpsCourtAppearanceId" to additionalInformation.courtAppearanceId,
  "offenderNo" to eventOffenderNo(personReference),
  "isBreach" to (additionalInformation.isBreach == true).toString(),
)
...
fun CourtSentencingService.SentenceCreatedEvent.asTelemetry() = mutableMapOf(
  "dpsCourtCaseId" to additionalInformation.courtCaseId,
  "dpsSentenceId" to additionalInformation.sentenceId,
  "dpsCourtAppearanceId" to additionalInformation.courtAppearanceId,
  "offenderNo" to eventOffenderNo(personReference),
)

fun CourtSentencingService.PeriodLengthCreatedEvent.asTelemetry() = mutableMapOf(
  "dpsCourtCaseId" to additionalInformation.courtCaseId,
  "dpsSentenceId" to additionalInformation.sentenceId,
  "dpsCourtAppearanceId" to additionalInformation.courtAppearanceId,
  "dpsTermId" to additionalInformation.periodLengthId,
  "offenderNo" to eventOffenderNo(personReference),
  "isBreach" to (additionalInformation.isBreach == true).toString(),
)

fun CourtSentencingService.RecallEvent.asTelemetry() = mutableMapOf(
  "dpsRecallId" to additionalInformation.recallId,
  "dpsPreviousRecallId" to (additionalInformation.previousRecallId ?: ""),
  "dpsSentenceIds" to additionalInformation.sentenceIds.joinToString(),
  "offenderNo" to eventOffenderNo(personReference),
)
