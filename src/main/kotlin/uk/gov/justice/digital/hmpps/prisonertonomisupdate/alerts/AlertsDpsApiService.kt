...
class AlertsDpsApiService(@Qualifier("alertsApiWebClient") private val webClient: WebClient) {
  suspend fun getAlert(alertId: String): Alert = webClient
    .get()
    .uri("/alerts/{alertId}", alertId)
    .retrieve()
    .awaitBody()

  suspend fun getActiveAlertsForPrisoner(offenderNo: String): List<Alert> = webClient
    .get()
    .uri {
  ...
  suspend fun getAllAlertsForPrisoner(offenderNo: String): List<Alert> = webClient
    .get()
    .uri {
      it.path("/prisoners/{offenderNo}/alerts")
        .queryParam("page", 0)
