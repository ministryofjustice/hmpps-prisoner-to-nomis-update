...
@Service
class TransferSchedulerReconciliationServiceAllPrisoners(
  ...
  suspend fun generateTransferSchedulerReconciliationReportBatch() {
    ...
    runCatching { generateTransferSchedulerReconciliationReport() }
      .onSuccess {
        log.info("Transfer scheduler all prisoners reconciliation report completed with ${it.mismatches.size} mismatches")
        telemetryClient.trackEvent(
  ...
  suspend fun checkPrisonersMatch(prisonerId: PrisonerId): PrisonerId? = runCatching {
    reconciliationService.checkPrisonersMatch(prisonerId.offenderNo).takeIf { it.isNotEmpty() }
      ?.let { prisonerId }
  }.onFailure {
