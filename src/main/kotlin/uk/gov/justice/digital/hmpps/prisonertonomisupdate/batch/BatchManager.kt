...
class BatchManager(
  ...
  private fun removeQueueListeners(event: ContextRefreshedEvent) {
    with(event.applicationContext.autowireCapableBeanFactory) {
      getBeanProvider<DomainEventListener>().forEach {
        this.destroyBean(it)
      }
    }
  }
